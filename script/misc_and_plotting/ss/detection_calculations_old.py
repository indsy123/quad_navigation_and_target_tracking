...
class detection_calculations(object): 
    def __init__(self, odometry_topic, goal, freq, a, beta, chi, alpha):
        """initialization"""
	...
	open(self.path + 'detection_data.txt', 'w').close()
        ...
        self.dt_detection = (1/freq)*20.0 # because I am doing calculations every 0.2 seconds
    ...
    def detection_callback(self, data):
        ...
        if self.detection_counter%(self.odometry_freq/5.0) == 0.0: # because I am doing calculations every 0.2 seconds
		...
		LR = np.exp(vi_integration)
                self.LR_vector.append(LR)
            print 'total number of counts received:', len(tau), vi_integration, count_multiplication

            if p == 0: 
                self.pmd_exponent = 0.0; self.detection_threshold = 0.0
                #self.vi_integration = self.vi_integration + (1-mu)*self.dt_detection*self.background
	    ...
	    f.write("%2.2f, %2.2f, %2.2f, %2.2f, %2.2f, %2.2f, %2.2f, %2.2f, %2.2f, %2.2f\n" % (self.time_vector[-1], mu, p, pfa, np.exp(self.pmd_exponent), len(tau), vi_integration, count_multiplication, LR, self.detection_threshold))
	    ...
	    #print 'detection parameters:', 'LR:',self.LR_vector[-1], 'threshold:',self.detection_threshold_vector[-1],'mu:', mu
            #if self.detection_counter == 0: 
...
if __name__ == '__main__':
    ...
    #r = rospy.Rate(30)    
    traj = detection_calculations(odometry_topic, goal, freq, a, beta, chi, alpha)
