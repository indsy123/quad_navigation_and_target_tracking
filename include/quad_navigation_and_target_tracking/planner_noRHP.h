...
class planner
{
  public: 
    ...
    void odometryCb1(const nav_msgs::Odometry::ConstPtr& msg);
    void odometryCb2(const nav_msgs::Odometry::ConstPtr& msg);
  ...
  private:     
    ...
    unsigned int odom1_counter, odom2_counter, wp_counter;

    double PI = 3.1416;
    double odom_frequency, mass, maxT, maxV, avgV, maxAngV, _bat, percentage;
    double hHover, Distance2Goal;
    double delt, T;
    double currentYaw, currentYawrate, desiredYaw, desiredYawrate;
    double tau_rp, tau_yaw;
    double current_time;

    ros::Subscriber odom_sub1, odom_sub2, bat_sub;
    ros::Publisher cmd_pub, best_traj_pub, pose_pub;

    string mode, odom_topic, cmd_topic, px4_controller_type, posecmd_topic, run_number, battery_topic;
    
    VectorXd initialState, currentState, desiredState, goalState, px, py, pz;
