...
class planner
{
  public: 
    planner(ros::NodeHandle* nodehandle) 
    {
      ...
      outdata2.open("ensamble.txt", std::ios::out|std::ios::binary);
      getParameters();
    ...
    void _factoredVelocity(double& time_diff, Vector3d& targetPose, VectorXd& currentState, double& safetyDistance, double& maxV, double& maxVel);
  private:     
    ros::NodeHandle nh_;
    ...
    double PI = 3.1416;
    double odom_frequency, pc_frequency, maxR, minR, xFOV, yFOV, mass, maxT, maxV, avgV, maxAngV, _bat, percentage;
    ...
    ros::Subscriber odom_sub, pc_sub, target_sub, bat_sub;
    ros::Publisher cmd_pub, pc_pub, ensemble_pub, best_traj_pub, selected_traj_pub, pose_pub;

    string mode, odom_topic, pc_topic, cmd_topic, target_topic, pcFrame, camBaseframe, run_number, px4_controller_type, posecmd_topic, battery_topic;
    
    Eigen::VectorXd initialState, currentState, desiredState, goalState, px, py, pz, localGoal;
    Eigen::Vector4d yaw_coeff;
    Eigen::Vector3d targetPose, targetVel;
    Matrix4d cogTbase, baseTcoc, cogTcoc, dronePose;

    std::vector <double> iPosition, final_goal, kx, kv, kR, kOmega, cgTb, bToc, local_goal, initialHoveringPosition;
    ...
    bool replan = false;
    bool RHPmode = true;
    bool targetpose_available = false;
