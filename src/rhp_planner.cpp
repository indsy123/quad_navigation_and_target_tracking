...
void planner::getParameters()
{
  ...
  nh_.param<double>("max_thrust", maxT, 27.0);
  ...
  outdata.open("subscribed_trajectory"+run_number+".txt", std::ios::out|std::ios::binary);
  outdata1.open("generated_trajectory"+run_number+".txt", std::ios::out|std::ios::binary);
  outdata2.open("other_parameters"+run_number+".txt", std::ios::out|std::ios::binary);
...
void planner::pointcloud_callback(const sensor_msgs::PointCloud2ConstPtr& cloud_in)
{
  ...
  if (waypointReached || pc_counter == 0 || replan) 
  {
    ...
    if (wp_counter == 0 || wp_counter == waypointList.size()-1) // initial hover and final land
    {
      ...
      if (wp_counter == 0)
      {
        goalState << initialState(0), 0.0, 0.0, 0.0,
                     ...
                     initialState(8)+hHover, 0.0, 0.0, 0.0; 
      }
      else
      {
        goalState << initialState(0), 0.0, 0.0, 0.0,
      ...
      //if (abs(goalState(0) - initialState(0))<0.03 && abs(goalState(4) - initialState(4))<0.03)
      //{desiredYaw = 0;}
      //else 
      //{
      ...
      //cout <<"1:"<<", wp_counter:"<<wp_counter<<", waypointList.size():"<<waypointList.size()<<endl;
    ...
    else //receding horizon plannning 
    { 
      ...
      convergance = sqrt(pow(goalState(0)-initialState(0), 2) + pow(goalState(4)-initialState(4), 2) + pow(goalState(8)-initialState(8), 2));
	  ...
	  //desiredYaw = atan2(localGoal(4) - initialState(4), localGoal(0) - initialState(0));
          ...
          //cout <<"2:"<<", wp_counter:"<<wp_counter<<", waypointList.size():"<<waypointList.size()<<endl;
          replan = true;
      ...
      else
      {
        ...
        double v = local_goal[3];
        //cout <<"v:"<< v<<endl;
	//cout << "initial state:"<< initialState(0)<<","<< initialState(4)<<","<< initialState(8)<<endl;
        ...
        //trajectory.getControlHorizonPoint(px, py, pz, controlHorizonTime, initialState);
        finalTrajectory(px, py, pz, yaw_coeff, T0, T);
        ros::Duration(T+3.0).sleep();
        //cout <<"3:"<<", wp_counter:"<<wp_counter<<", waypointList.size():"<<waypointList.size()<<endl;
        wp_counter +=1;
        initialState << waypointList[wp_counter-1][0], 0.0, 0.0, 0.0, 
                        waypointList[wp_counter-1][1], 0.0, 0.0, 0.0, 
                        waypointList[wp_counter-1][2], 0.0, 0.0, 0.0;
